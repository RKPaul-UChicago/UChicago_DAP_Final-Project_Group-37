...
#| fig-cap: "Precinct-level voter turnout rate across all five election years (2008–2024)"

def make_turnout_choropleth(gdf, year):
    # Convert GeoDataFrame to GeoJSON dict — Altair renders this reliably
    ...
    return (
        ...
        .properties(width=180, height=280, title=str(year))
    )
...
```{python}
# Socioeconomic Index Construction
# 1) Standardized Z-score index (within election year)
# 2) PCA-based index (within election year)
# Keeps BOTH in master_idx

# Required columns in `master`:
# election_year, precinct_id, turnout_rate,
# median_hh_income, pct_college, pct_renter, pct_18_29
...
# Method 1: Z-score SES index
z_vars = ["median_hh_income", "pct_college", "pct_renter", "pct_18_29"]

# Create within-year z scores
master_idx["income_z"]  = master_idx.groupby("election_year")["median_hh_income"].transform(zscore_safe)
master_idx["college_z"] = master_idx.groupby("election_year")["pct_college"].transform(zscore_safe)
master_idx["renter_z"]  = master_idx.groupby("election_year")["pct_renter"].transform(zscore_safe)
master_idx["youth_z"]   = master_idx.groupby("election_year")["pct_18_29"].transform(zscore_safe)

# Orient so higher = more advantage (income, college positive; renter, youth negative)
master_idx["ses_z"] = (
    master_idx["income_z"] +
    master_idx["college_z"] -
    master_idx["renter_z"] -
    master_idx["youth_z"]
) / 4

# Rescale within-year to 0-100 for interpretability so precincts with higher index are interpreted more socio economically advantaged.

master_idx["ses_z_0_100"] = master_idx.groupby("election_year")["ses_z"].transform(minmax_safe) * 100


# Method 2: PCA SES index
...
for yr, df in master_idx.groupby("election_year"):
    ...
    # Orient so higher = more advantage by flipping disadvantage variables
...
### Justification for constructing index using PCA 
PCA is producing stronger turnout gradients because PCA is effectively weighting:
Education and income heavily
Youth share moderately
Renter share very little

In contrast, equal weight z index gives renter equal importance, which dilutes the gradient. Evidently, pct_renter loading is slightly negative, but tiny. That means renter share is basically not contributing meaningfully to SES once education and income are accounted for.

That explains why PCA correlates more strongly with turnout

In sum:
	•	The SES index is primarily driven by education and income.
	•	Education is the strongest structural gradient in Chicago turnout.
	•	Youth concentration depresses turnout.
	•	Housing tenure plays a much smaller role than commonly assumed.
...
print([c for c in master_idx.columns if "ses" in c.lower()])

# 2) Correlation with turnout by year
corr_by_year = master_idx.groupby("election_year").apply(
    lambda d: d[["ses_z", "turnout_rate"]].dropna().corr().iloc[0, 1]
).rename("corr_ses_z_turnout")

corr_by_year_pca = master_idx.groupby("election_year").apply(
    lambda d: d[["ses_pca", "turnout_rate"]].dropna().corr().iloc[0, 1]
).rename("corr_ses_pca_turnout")

print("\nCorrelation with turnout by year:")
print(pd.concat([corr_by_year, corr_by_year_pca], axis=1))

# 3) Quintile turnout table for z-index (0-100)
tmp_z = master_idx.dropna(subset=["ses_z_0_100", "turnout_rate"]).copy()
tmp_z["ses_q_z"] = tmp_z.groupby("election_year")["ses_z_0_100"].transform(
    lambda s: pd.qcut(s, 5, labels=False) + 1
)
turnout_by_q_z = (tmp_z.groupby(["election_year", "ses_q_z"])["turnout_rate"].mean() * 100).rename("avg_turnout_pct")
print("\nTurnout by SES quintile (Z index):")
print(turnout_by_q_z)

# 4) Quintile turnout table for PCA index (0-100)
tmp_p = master_idx.dropna(subset=["ses_pca_0_100", "turnout_rate"]).copy()
tmp_p["ses_q_pca"] = tmp_p.groupby("election_year")["ses_pca_0_100"].transform(
    lambda s: pd.qcut(s, 5, labels=False) + 1
)
turnout_by_q_pca = (tmp_p.groupby(["election_year", "ses_q_pca"])["turnout_rate"].mean() * 100).rename("avg_turnout_pct")
print("\nTurnout by SES quintile (PCA index):")
print(turnout_by_q_pca)

# PCA is much more tightly aligned with turnout rate.

```
```{python}
### PCA loadings
### PCA loadings are the weights that PCA assigns to each variable when constructing the principal component. They show how strongly each variable contributes to the index.

vars_pca = ["median_hh_income", "pct_college", "pct_renter", "pct_18_29"]

for yr, df in master_idx.dropna(subset=vars_pca).groupby("election_year"):
    X = df[vars_pca].copy()
    X["pct_renter"] = -X["pct_renter"]
    X["pct_18_29"]  = -X["pct_18_29"]
    ...
    loadings = pd.Series(pca.components_[0], index=vars_pca).sort_values(key=np.abs, ascending=False)
    print("\nYear:", yr)
    print(loadings.round(3))
```
```{python}
# Turnout vs SES (PCA Index) by year
...
pts = (
    base
    .mark_circle(size=14, opacity=0.6)
    .encode(
...
### linearmodels PanelOLS: When the SES of a given precinct improves relative to its own history, does turnout increase?
```{python}
fe_df = master_idx[[
    "precinct_id",
    "election_year",
...
Turnout_{it} = \beta SES_{it} + \alpha_i + \gamma_t + \epsilon_{it}

Where
	•	i = precinct
	•	t = election year
	•	\alpha_i = precinct fixed effects
	•	\gamma_t = year fixed effects

This controls for:
	•	persistent neighborhood characteristics
	•	election-specific shocks affecting the entire city

So, the estimate comes from within-precinct changes over time. This makes the estimate much more credible than a simple correlation.

### Results & Interpretation
Coefficient on SES (PCA)
β = 0.0335
Std. Error = 0.0019
Within R² = 0.0789
t-stat = 17.6
p-value = < 0.001  #This means the relationship is highly statistically significant.
F-statistic (robust) = 311
p-value < 0.001

A 1 standard deviation increase in precinct socioeconomic status increases turnout by about 3.35 percentage points.

More intuitively: If a precinct becomes 1 SD more advantaged, turnout increases by ~3.3 percentage points.

That is a large effect in voting behavior.

R² means SES explains about 7.9% of within-precinct turnout variation over time.

That is actually reasonable because turnout is driven by many things:
	•	campaign intensity
	•	candidate competitiveness
	•	mobilization
	•	demographic change

But SES still explains a meaningful share.

Moreover, the model overall is highly significant from the F-stat.
