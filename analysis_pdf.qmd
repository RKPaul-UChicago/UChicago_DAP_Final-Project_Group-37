...
8. **Part 8 – Analysis:** Bar chart, choropleth maps, regression analysis, scatter plots, income quintile plot.
9. **Part 9 – Policy Implications and Recommendations:** Interpret findings and connect to policy.

---
## Research Question and Motivation

Presidential elections in Chicago are characterized by sufficiently high levels of participation, yet voter turnout is far from evenly distributed across the city. Some neighborhoods vote at much higher rates than others, and these differences persist from election to election. These patterns matter because turnout shapes whose preferences are represented in democratic decision-making. If participation varies systematically across neighborhoods, the electorate may not fully reflect the broader population of the city.

This project examines who votes in Chicago and how representative the electorate is across neighborhoods during presidential elections from 2008 to 2024. In particular, we are interested in whether turnout patterns remain stable across time and whether higher-turnout elections reduce participation gaps or simply raise turnout everywhere. Understanding these dynamics is important for evaluating civic engagement in the city and identifying where barriers to participation may still exist.

Chicago provides an especially compelling setting for this analysis. The city contains diverse neighborhoods with large differences in income, education, housing stability, and demographic composition. These characteristics are likely to influence political participation. By studying turnout patterns across precincts and over time, we aim to better understand how neighborhood characteristics relate to voter participation and what these patterns suggest about the representativeness of the electorate.

Our analysis is descriptive and policy-oriented rather than causal, but it highlights important structural patterns in turnout that may inform civic engagement policies and voter outreach efforts.
...
To study neighborhood-level voting patterns, we combine several datasets that capture both electoral participation and local demographic characteristics.

First, we use precinct-level voter turnout data from the Chicago Board of Elections, which provide counts of registered voters and ballots cast for presidential elections from 2008 through 2024. These data allow us to measure turnout rates at the precinct level for each election year.

Second, we incorporate demographic and socioeconomic data from the American Community Survey (ACS). We use ACS 5-year estimates aligned to each election year in order to approximate neighborhood conditions during that period. The demographic variables we focus on include median household income, educational attainment, renter share, racial composition, and the share of young residents.

Finally, we use geographic boundary data from two sources:
	•	Chicago precinct boundary files from the Chicago Data Portal
	•	Census tract shapefiles from the U.S. Census TIGER/Line program

These spatial datasets allow us to link demographic information to voting precincts.

Because census tracts and voting precincts do not share identical boundaries, demographic data cannot be merged directly onto precinct election results. To address this, we apply area-weighted spatial interpolation. This method calculates the geographic overlap between census tracts and precinct polygons and assigns tract-level demographic characteristics to precincts in proportion to their area overlap. This approach allows us to approximate the demographic composition of each voting precinct while maintaining geographic consistency across election years.

The resulting dataset is a precinct-by-year panel, where each observation represents one precinct in one presidential election. The dataset includes turnout rates, registration counts, demographic indicators, and spatial identifiers.

To summarize overall neighborhood conditions, we construct a Socioeconomic Status (SES) index using Principal Component Analysis (PCA). The SES index combines several related variables into a single measure capturing overall socioeconomic advantage. Higher values indicate precincts with higher income, higher educational attainment, and lower housing instability.
...
Turnout is not evenly distributed across Chicago. Instead, clear geographic patterns emerge when turnout is mapped at the precinct level. Higher turnout precincts consistently appear in parts of the North Side and along the lakefront, while lower turnout is more common in parts of the South and West sides of the city. These spatial patterns appear in every election year in our dataset. Even during high-turnout presidential elections, the relative ranking of neighborhoods tends to remain similar. This persistence suggests that turnout inequality is not simply driven by a particular election or candidate. Instead, it reflects structural differences across neighborhoods that influence political participation.
...
A strong relationship emerges between socioeconomic conditions and voter participation. Across every election year, precincts with higher values on the socioeconomic status index consistently exhibit higher turnout rates. Conversely, precincts with lower SES scores tend to show lower participation. This pattern appears remarkably stable across the entire 2008–2024 period. These results suggest that socioeconomic advantage is strongly associated with civic participation. Factors such as educational attainment, income stability, and youth share likely shape both the resources and the institutional familiarity that make participation easier.
...
To quantify the relationship between neighborhood socioeconomic conditions and turnout, we estimate a two-way fixed effects panel model that controls for both precinct-specific and election-year effects. The results indicate a positive and statistically significant relationship between the SES index and turnout. Interpreting the coefficient in substantive terms, the results suggest that if a precinct becomes one standard deviation more socioeconomically advantaged, turnout increases by roughly 3.3 percentage points. The finding reinforces the descriptive patterns observed in the visualizations.

### 8.3 Two-way Fixed Effects Panel Regression

When the SES of a given precinct improves relative to its own history, does turnout increase?
If a precinct becomes 1 SD more advantaged, turnout increases by ~3.3 percentage points. That is a large effect in voting behavior.
```{python}
...
---
### Demographic correlations over time

Examining correlations between turnout and demographic variables reveals additional insights about participation patterns. Education and income show the strongest and most consistent positive relationships with turnout across election years. Precincts with higher shares of college-educated residents and higher household income levels consistently exhibit higher voter participation. In contrast, precincts with larger shares of young residents or renters tend to exhibit lower turnout. These patterns are consistent with broader national trends in political participation.Despite fluctuations across elections, the overall structure of turnout differences remains remarkably consistent across time.

### 8.4 Correlation Trends against Demographhics Over Time

```{python}
#| label: fig-corr-trends
#| fig-cap: "How turnout–demographic correlations change across election years"
...
for year in ELECTION_YEARS:
    ...
    for col in DEMO_COLS:
        ...
        if len(valid) >= 30:
            r = valid["turnout_rate"].corr(valid[col], method="spearman")
            row[col] = round(r, 3)
        else:
            row[col] = np.nan
...
chart_trends = (
    ...
    .encode(
        ...
        y=alt.Y(
            "Spearman_r:Q",
            title="Spearman r",
            scale=alt.Scale(domain=[-0.65, 1.0])
        ),
        color=alt.Color(
            "Variable:N",
            title="Demographic",
...
display_altair_png(chart_trends_final, scale=3)
```
...
The analysis identifies associations rather than causal effects. While socioeconomic characteristics are strongly correlated with turnout, the analysis does not establish that these variables directly cause differences in participation.

Secondly, the demographic characteristics of precincts are estimated using area-weighted spatial interpolation from census tracts. Although this is a standard approach when geographic units do not align perfectly, it introduces some measurement uncertainty into the demographic estimates.

Finally, voter turnout is influenced by many factors not captured in this dataset, including campaign mobilization efforts, candidate characteristics, and historical voting behavior.

## Policy Implications and Recommendations

The results highlight persistent inequalities in political participation across Chicago neighborhoods. Lower-socioeconomic areas consistently exhibit lower turnout, suggesting that structural barriers to participation remain an important issue.

Education appears to be the most consistent predictor of turnout differences. This suggests that turnout inequality may reflect differences in civic knowledge, access to political information, and familiarity with voting institutions rather than purely financial constraints.

These findings point toward several potential policy directions.

Community-based voter outreach efforts may help increase participation in lower-SES precincts by improving access to information and reducing logistical barriers to voting. Civic education programs that explain voting processes and emphasize the importance of participation may also play an important role, particularly for younger residents.

Institutional reforms that reduce participation barriers—such as early voting opportunities and same-day voter registration—may further help expand access to the electoral process.

Ultimately, improving the representativeness of the electorate requires reducing structural gaps in civic engagement across neighborhoods. By identifying where turnout gaps are most persistent, this analysis provides a foundation for more targeted efforts to strengthen democratic participation across Chicago.
